...
void MemoryManager::readAll() {
   ...
   if (hSnapshot != INVALID_HANDLE_VALUE) {
      ...
      if (Process32First(hSnapshot, &ProcEntry)) {
         ...
         std::cout << "Process found ! Executable name : " << currentProcessName << std::endl;

         HANDLE moduleSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, currentProcessId);
         if (moduleSnapshot != INVALID_HANDLE_VALUE) {
            MODULEENTRY32 ModEntry;
            ModEntry.dwSize = sizeof(MODULEENTRY32);

            if (Module32First(moduleSnapshot, &ModEntry)) {
               std::cout << "-- -- -- Module found ! Module name : " << ModEntry.szModule
                     << ", module path: " << ModEntry.szExePath << std::endl;
